...
public interface PetRepository extends JpaRepository<Pet, Integer> {
    ...
    @Query("select new Pet(p.serialNumber, p.birthDate, p.race, p.sex, p.type, p.is_approved, p.ownerCode) from Pet p join Citizen c on p.ownerCode = c.id where c.username= ?1 and p.is_approved = 0")
